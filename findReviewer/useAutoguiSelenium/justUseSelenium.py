...
# @Time    : 11/15/24 11:58
# @Author  : lzg
# @Site    : 一样无法无法弹出验证码，这策略太狠了，应该是检测到了selenium的特征
# @File    : justUseSelenium.py
# @Software: PyCharm
import random
import time
...
class Scholar:
    ...
    def __search_onepage(self):
        ...
        if not self.check_element_exist(check_type='ID', value='gs_res_ccl_mid'):
            print('>> 当前页面不存在文章列表')
            return []
        gs_scl = self.driver.find_element(by=By.ID, value='gs_res_ccl_mid').find_elements(by=By.CLASS_NAME,
                                                                                          value='gs_scl')
        for i, item in tqdm(enumerate(gs_scl)):
            ...
            gs_rt_a = gs_rt.find_element(by=By.TAG_NAME, value='a') if self.check_element_exist(check_type='TAG_NAME',
                                                                                                value='a',
                                                                                                source=gs_rt.get_attribute(
                                                                                                    'innerHTML')) else None
    ...
    def search(self, keywords, sort_bydate=False, as_ylo='', as_yhi='', max_pages=100, delay=0):
        ...
        total_num = self.driver.find_element(by=By.ID, value='gs_ab_md').find_element(by=By.CLASS_NAME,
                                                                                      value='gs_ab_mdw').text.strip()  # .replace('\n', '').split(',')[:-1]
    ...
    def save_file(self, filename='scholar.xlsx', nodup=False):
        ...
        try:
            pd.DataFrame(unique_data).dropna().reset_index(drop=True).to_excel(
                os.path.join(self.out_filepath, filename), index=False, encoding='utf-8')
        except Exception as e:
            if pyautogui.confirm(
                    text=f'文件保存失败[{str(e)}]\n点击“确定”将内容复制到剪切板;\n否则, 点击“取消”直接结束脚本;',
                    title='文件保存失败', buttons=['确定', '取消']) == '确定':
...
class AnalyzeDraw:
    ...
    def draw_wordcloud(self):
        ...
        # 提取英文标题并排除非英文内容
        english_titles = self.df['title'].apply(lambda x: ' '.join([word.lower() for word in nltk.word_tokenize(x) if
                                                                    word.isalpha() and word.lower() not in english_stopwords]))
        # 将所有英文标题合并为一个字符串
...
if __name__ == '__main__':
    ...
    results = scholar.search(keywords, sort_bydate, as_ylo, as_yhi, max_pages=int(max_pages),
